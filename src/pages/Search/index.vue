    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="details clearfix">
         ...
         <Pagination :pageSize="searchParams.pageSize" :pageNo="searchParams.pageNo" :continues="5" :total="total" @getPageNo="getPageNo"/>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  // 在组件挂载完毕之前执行一次
  beforeMount() {
    //在发请求之前，把接口需要传递的数据进行整理
    Object.assign(this.searchParams,this.$route.query,this.$route.params);
  },
  mounted() {
    // 在发请求前给服务器发送参数【searchParams参数发生变化有数值带给服务器】
    this.getData();
  },
  methods: {
    //向服务器派发actions请求 (根据参数不同 返回不同数据 渲染页面)
    //封装为函数 需要时调用即可
    getData() {
    ...
    removeCategoryName(){
      ...
      this.getData();
      //地址栏也需要更改，进行路由跳转
      // 严谨：本意是删除query参数，如果路径中出现params参数 不应该删除
